...
opts_chunk$set(fig.width=6, fig.height=5, fig.path='figure/',
...
Fyke-net traps and hoop traps were used to sample Black Bear Lake. Rainbow trout (Oncorhynchus mykiss) captured in the gear were marked to facilitate a mark-recapture experiment to estimate trout abundance. An estimated `r round((N),0)` (SE = `r round((se), 2)`) rainbow trout >= 180 mm FL were present in 2002.
...
```{r plot1_SEAK, fig.cap="Map of Southeast Alaska, showing location of Black Bear Lake on Prince of Wales Island."}
library(maps)
library(mapdata)
...
map.scale(-140.5,60, ratio=FALSE, relwidth=0.3, cex=1.0)
seAK_lat <- 55.541175
seAK_lon <- -132.855405
...
```{r plot2_BBL, fig.cap="Black Bear Lake, Prince of Wales Island, SE Alaska."}
library(ggmap)
...
```{r plot3_LenghFreq, fig.width=4.5, fig.height=4, fig.cap="Length-frequency distribution of rainbow trout in Black Bear lake, July 2002."}
...
```{r plot4_ScatterPlot, fig.width=4.5, fig.height=4, fig.cap="Rainbow Trout Length (mm) and Weight (g)"} 
scatter <-function(rbt, Length_mm, Weight_g, RBT) {
  require(ggplot2)
...
scatter(rbt, "Length_mm", "Weight_g", "Rainbow Trout")
```
...
```{r plot5_LM, fig.width=4.5, fig.height=4, fig.cap="Natural Log transformed length and  weight during July, 2002."}
require(ggplot2) 
# Regression of length on weight for all fish > 60 mm
rbt_sub <- subset(rbt, Length_mm > 60 & Recapture == "n")

linear <- qplot(log(Length_mm), log(Weight_g), data=rbt_sub, geom=c("point", "smooth"), 
      method="lm", formula=y~x,
      main="Regression of Weight on Length", 
      xlab="Ln Length (mm)", ylab="Ln Weight (g)")
```

```{r plot6_CumDist, fig.width=4.5, fig.height=4,fig.cap="Cumulative distribution of lenght of rainbow trout new capture enent 1, and new capture event 2." }
rbt_180 <- subset(rbt, Length_mm > 180 & Recapture == "n")

df <- data.frame(Length = rbt_180$Length_mm, Event = factor(rbt_180$CaptureEvent))
df <- na.omit(df) 

p8 <- ggplot(df, aes(Length, group=Event))
  p8 <- p8 +  stat_ecdf(aes(color=Event))
  p8 <- p8 + labs(title ="Cum Length Dist", x="Length", y = "Cumulative Density")
  p8 <- p8 + theme_bw()
p8
```
...
```{r table1_LM, echo=FALSE, tidy=TRUE}
lmfit <- lm(log(rbt_sub$Weight_g) ~ log(rbt_sub$Length_mm))
summary(lmfit)
```

<br/>

```{r plot7_Residuals, echo=FALSE, fig.width=3.5, fig.height=3, fig.cap="Linear fit model residuals plots."}
plot(lmfit)
par(mfrow=c(2,2))
xlab="Weight~Length"
```
...
```{r table2_summary, echo=FALSE}
# Summary Table of Fish Length (mm)
length <-na.omit(rbt$Length_mm)
summary(length)

# Summary Table of Fish Weight (g)
weight <- na.omit(rbt$Weight_g)
summary(weight)
```
...
```{r plot7_FultonK, fig.cap="Fulton Conditon frequency histogram."}
K <-  na.omit(rbt$FultonK)
...
In 2002, Black Bear Lake had the sixth largest (N = `r round((N), 0)`) rainbow trout population on record during the 7 years of monitoring. This is a conservative estimate since fish < 180 mm were not included in the estimate. From our observations it is felt that the true population value probably lies on the upper side of the population estimate, possibly in the region of 1,000 to 1,100 fish.
...
# The below code converts markdown file to pdf, or Word docx using the pander package.

require(pander)
Pandoc.convert(f="BBLreport.md", format="pdf", options="-s --toc -V geometry:margin=1in")
