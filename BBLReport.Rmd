...
opts_chunk$set(fig.width=7, fig.height=6, fig.path='figure/',
...
```{r plot3_BBL, fig.cap="Black Bear Lake, Prince of Wales Island, SE Alaska."}
library(ggmap)
...
```{r plot4_LenghFreq, fig.cap="Length-frequency distribution of rainbow trout in Black Bear lake, July 2002."}
rbt_sub <- subset(rbt, Length_mm & Recapture == "n")
p4 <- hist(rbt_sub$Length_mm, breaks = 40, 
     xlim = c(min(rbt_sub$Length_mm), max(rbt_sub$Length_mm)), 
     freq = FALSE, 
     col = 'lightblue',
     main = 'Trout Lenght Distribution',
     xlab = 'Length')
lines(density(rbt_sub$Length_mm, bw=10), col = 'red', lwd = 2)
box()
```


```{r plot5_ScatterPlot, fig.cap="Rainbow Trout Length (mm) and Weight (g)"} 
...
```{r plot6_LM, fig.cap="Natural Log transformed length and  weight during July, 2002."}
require(ggplot2) 
...
```{r plot7_Residuals, fig.cap="Linear fit model residuals plots."}
plot(lmfit)
par(mfrow=c(2,2))
```

**Table 2.  Summary table for fish lenght in mm (top), and weight in grams (bottom).**
```{r table2_summary, eval=FALSE, echo=FALSE}
# Summary Table of Fish Length (mm)
l <-na.omit(rbt$Length_mm)
summary(l)
# Summary Table of Fish Weight (g)
w <- na.omit(rbt$Weight_g)
summary(w)
```
...
```{r plot8_CumDist, fig.cap="Cumulative distribution of lenght of rainbow trout new capture enent 1, and new capture event 2." }
rbt <- subset(rbt, Length_mm > 60 & Recapture == "n")
df <- data.frame(Length = rbt$Length_mm, Event = factor(rbt$CaptureEvent))
df <- na.omit(df) 

p <- ggplot(df, aes(Length, group=Event))
p <- p +  stat_ecdf(aes(color=Event))
p <-p+ labs(title ="Cum Length Dist", x="Length", y = "Cumulative Density")
p
```
...
```{r plot9_FultonK, fig.cap="Fulton Conditon frequency histogram."}
K <-  na.omit(rbt$FultonK)
...
# require(pander)
# Pandoc.convert(f="BBLreport.md", format="pdf", options="-s")
